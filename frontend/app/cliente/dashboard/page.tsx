...
export default function ClienteDashboard() {
  ...
  const upcomingAppts = appointments?.filter((a: Appointment) => isFuture(parseISO(a.start_datetime)) && a.status !== 'cancelada').sort((a: Appointment, b: Appointment) => new Date(a.start_datetime).getTime() - new Date(b.start_datetime).getTime());
  const pastAppts = appointments?.filter((a: Appointment) => isPast(parseISO(a.start_datetime)) || a.status === 'cancelada').sort((a: Appointment, b: Appointment) => new Date(b.start_datetime).getTime() - new Date(a.start_datetime).getTime());
      ...
      <div className="min-h-screen bg-background text-textMain">
        ...
        <main className="max-w-6xl mx-auto px-4 py-12">
          <div className="mb-8 border-b border-border pb-4 flex justify-between items-end">
            <div>
              <h1 className="text-3xl font-display font-bold text-primary mb-2">Panel del Cliente</h1>
                        ...
                        <CardContent className="p-5">
                          ...
                          <div className="bg-surfaceLayer rounded-lg p-4 flex flex-col sm:flex-row justify-between items-center sm:items-start gap-3">
                            ...
                            <div className="text-right">
                              <p className="text-sm text-textMuted line-through">Duración: {appt.service_duration} min</p>
                              <p className="text-primary font-semibold text-lg">{appt.price_at_booking} €</p>
                            </div>
                          </div>
                      ...
                      <Card key={appt.id} className="border-border bg-surfaceLayer/30 opacity-70">
                        <CardContent className="p-5">
                          ...
                          <div className="flex justify-between items-center text-sm">
                            ...
                            <span className="font-semibold text-white">{appt.price_at_booking} €</span>
                          </div>
                        </CardContent>
                      </Card>
                    ))}
